            ...
            <div class="p-4">
               <div class="flex flex-wrap items-center justify-between mb-2">
                    <div class="text-sm text-gray-500">
                       <a href="archiveemployee.php" class="btn btn-outline-primary btn-sm d-flex align-items-center gap-1">
...
<script>
  ...
  document.addEventListener("DOMContentLoaded", () => 
 {
    ...
    `;
    fetch(`${APP_URL}/api/employeecomplete`, {
        headers: {
            "Accept": "application/json",
            "Authorization": "Bearer " + token
        }
    ...
    .then(data => {
        employees = data.filter(emp => !emp.is_archived);
        renderTable(employees);
    })
    .catch(err => {
        console.error("Error loading employee data:", err);
        tableBody.innerHTML = `
            <tr><td colspan="6" class="py-4 px-4 text-red-500 text-center">
                Failed to load data.
            </td></tr>`;
   });
    function renderTable(list) {
        ...
        if (list.length === 0) {
            tableBody.innerHTML = `
                <tr><td colspan="6" class="py-4 px-4 text-gray-500 text-center">
                    No employees found.
                </td></tr>`;
            return;
        }
            ...
            tr.innerHTML = `
                <td class="py-2 px-4">
                    <button type="button" class="btn btn-warning btn-sm"
                            onclick="archiveEmployee(${emp.id}, event)" title="Archive">
                        <i class="bi bi-archive"></i>
                    </button>
                    <a href="viewemployee.php?id=${emp.id}"   title="View employee">
                        <i class="ri-eye-line text-blue-500 hover:text-blue-700 cursor-pointer text-lg"></i>
                    </a>
                    <a href="updateemployee.php?id=${emp.id}" title="Edit employee">
                        <i class="ri-edit-line text-green-500 hover:text-green-700 cursor-pointer text-lg"></i>
                    </a>
                </td>
                <td class="py-2 px-4">${emp.employee_id}</td>
                <td class="py-2 px-4">${emp.full_name}</td>
                <td class="py-2 px-4">${emp.position}</td>
                <td class="py-2 px-4">${formatStatus(emp.status)}</td>
                <td class="py-2 px-4">${formatReason(emp.reason)}</td>`;
...
<script>
    function archiveEmployee(id, event) 
  {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            fetch(`${APP_URL}/api/employeearchive/${id}/archive`, {
                method: 'POST',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json',
            ...
            .then(data => {
                if (data.success) {
                    ...
                    Swal.fire('Error!', data.message || 'Failed to archive employee.', 'error');
                }
            })
            .catch(err => {
