            ...
            <header class="bg-white shadow-sm">
                ...
                <div class="px-6 py-2 border-t border-gray-100">
                    <div class="flex items-center text-sm">
                        <a href="#" class="text-gray-500 hover:text-primary">Archived</a>
...
document.addEventListener('DOMContentLoaded', () => 
{
    ...
    document.getElementById('search-student-id').addEventListener('input', e => {
        const q = e.target.value.trim().toLowerCase();
        if (q === '') {
            filteredList = studentList.slice();
        } else {
...
function loadArchivedStudents() 
{
    ...
    tableBody.innerHTML = `
        <tr><td colspan="6" class="text-center py-10">Loading archived students...</td></tr>
    `;

    fetch(`${APP_URL}/api/archived-students`, {
        headers: {
    ...
    .then(r => r.json())
    .then(students => {
        studentList  = Array.isArray(students) ? students : [];
        filteredList = studentList.slice();        
        currentPage  = 1;
        renderPage();
    })
    .catch(err => {
        ...
        tableBody.innerHTML = `
            <tr><td colspan="6" class="text-center py-10 text-red-500">
                Failed to load archived students.</td></tr>`;
    });
}
function renderPage() 
{
    ...
    const rows       = filteredList.slice(start, start + rowsPerPage);
        ...
        tr.innerHTML = `
            <td class="py-2 px-4">
                <button class="btn btn-success btn-sm"
                        onclick="unarchiveStudent(${s.id}, event)" title="Unarchive">
                    <i class="bi bi-arrow-up-circle"></i>
                </button>
            </td>
            <td class="py-2 px-4">${s.student_id ?? ''}</td>
            <td class="py-2 px-4">${s.first_name} ${s.last_name}</td>
            <td class="py-2 px-4">${s.course ?? ''}</td>
            <td class="py-2 px-4">${formatStatus(s.status)}</td>
            <td class="py-2 px-4">${formatReason(s.reason)}</td>`;
...
function unarchiveStudent(id, event) 
{
    if (event) event.preventDefault();
    Swal.fire({
        title: 'Are you sure?',
        text: 'You want to unarchive this student.',
        icon: 'info',
        showCancelButton: true,
        ...
        if (res.isConfirmed) {
            fetch(`${APP_URL}/api/unarchive/${id}/unarchive`, {
                method: 'POST',
                headers: {
                    'Accept': 'application/json',
                    'Authorization': `Bearer ${localStorage.getItem('auth_token')}`,
            ...
            .then(data => {
                if (data.success) {
                    Swal.fire('Success', 'Student unarchived successfully!', 'success');
                    setTimeout(loadArchivedStudents, 1000);
                } else {
                    Swal.fire('Error!', data.message || 'Failed to unarchive student.', 'error');
                }
            })
            .catch(() => Swal.fire('Error!', 'Something went wrong while unarchiving.', 'error'));
        }
    });
}
...
function formatStatus(status) 
{
    ...
    switch (status) {
        case 'first_print': return badge('First Print', '#3498db');
        case 're_print'   : return badge('Reprint'   , '#e67e22');
        default           : return badge(status      , '#7f8c8d');
    }
}
