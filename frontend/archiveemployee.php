...
document.addEventListener('DOMContentLoaded', () => 
{
    ...
    document.getElementById('search-employee').addEventListener('input', e => {
        ...
        if (q === '') {
            filteredList = employeeList.slice();   
        } else {
...
function loadArchivedEmployees()
{
    ...
    tableBody.innerHTML = `
        <tr><td colspan="6" class="text-center py-10">Loading archived employees...</td></tr>
    `;
    fetch(`${APP_URL}/api/archived-employee`, {
        headers: {
    ...
    .then(r => r.json())
    .then(emp => {
        employeeList  = Array.isArray(emp) ? emp : [];
        filteredList  = employeeList.slice();
        currentPage   = 1;
        renderPage();
    })
    .catch(err => {
        ...
        tableBody.innerHTML = `
            <tr><td colspan="6" class="text-center py-10 text-red-500">
                Failed to load archived employees.</td></tr>`;
    });
}
function renderPage() 
{
        ...
        tr.innerHTML = `
            <td class="py-2 px-4">
                <button class="btn btn-success btn-sm"
                        onclick="unarchiveEmployee(${employee.id}, event)" title="Unarchive">
...
function unarchiveEmployee(id, event) 
{
    if (event) event.preventDefault();
    Swal.fire({
        title: 'Are you sure?',
        text: 'You want to unarchive this employee.',
        icon: 'info',
        showCancelButton: true,
        ...
        if (res.isConfirmed) {
            fetch(`${APP_URL}/api/employeeunarchive/${id}/unarchive`, {
                method: 'POST',
                headers: {
                    Accept: 'application/json',
                    Authorization: `Bearer ${localStorage.getItem('auth_token')}`,
            ...
            .then(data => {
                if (data.success) {
                    Swal.fire('Success', 'Employee unarchived successfully!', 'success');
                    setTimeout(loadArchivedEmployees, 1000);
                } else {
                    Swal.fire('Error!', data.message || 'Failed to unarchive employee.', 'error');
                }
            })
...
function formatStatus(status) 
{
    ...
    switch (status) {
        case 'first_print':
            return badge('First Print', '#3498db');
        case 're_print':
            return badge('Reprint', '#e67e22');
         default:
            return badge(status, '#7f8c8d');
     }
}
