...
<link rel="stylesheet" href="assets/css/id.css">
        ...
        document.addEventListener("DOMContentLoaded", async () => 
    {
        ...
        try {
            const response = await fetch(`${APP_URL}/api/showemployeecomplete/${employeeId}`, {
                method: "GET",
                headers: {
                    "Accept": "application/json",
        ...
        if (data.image && studentImg) {
            studentImg.src = data.image.includes('http') ? data.image : `${APP_URL}/storage/${data.image}`;
        }
        if (data.signature && signatureImg) {
            signatureImg.src = data.signature.includes('http') ? data.signature : `${APP_URL}/storage/${data.signature}`;
        }
